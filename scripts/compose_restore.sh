...
if [ "$#" -ne 2 ] || [ -z "$1" ]; then
  printf '%s\n' "usage: scripts/compose_restore.sh BACKUP_DIR --confirm-destroy" >&2
  exit 64
fi
...
case "$tmp_root" in
  /*) ;;
  *)
    printf '%s\n' "TMPDIR must be an absolute directory" >&2
    exit 1
    ;;
esac

if [ ! -d "$tmp_root" ]; then
  printf '%s\n' "TMPDIR is not a directory: $tmp_root" >&2
  exit 1
fi
...
on_exit() {
  ...
  if [ "$writers_stopped" = true ] && [ "$status" -ne 0 ]; then
    ...
    if [ "$emergency_status" -eq 0 ]; then
      printf '%s\n' "restore failed; app and nginx remain stopped; repair the recovery set before restarting" >&2
    else
      printf '%s\n' "restore cleanup could not confirm app and nginx are stopped" >&2
  ...
  if [ -n "$staging_dir" ]; then
    for artifact in SHA256SUMS fornacast.dump fornacast-data.tgz; do
      if ! rm -f -- "$staging_dir/$artifact"; then
        cleanup_failed=true
      fi
    done

    if ! rmdir -- "$staging_dir"; then
      printf '%s\n' "could not remove private restore staging directory: $staging_dir" >&2
      cleanup_failed=true
    fi
  fi
...
for artifact in SHA256SUMS fornacast.dump fornacast-data.tgz; do
  if [ ! -f "$backup_dir/$artifact" ] || [ -L "$backup_dir/$artifact" ]; then
    printf '%s\n' "backup artifact must be a regular non-symlink file: $artifact" >&2
    exit 1
  fi
done

staging_dir=$(mktemp -d "$tmp_root/fornacast-restore-$expected_project.XXXXXX")
cp -P -- "$backup_dir/SHA256SUMS" "$staging_dir/SHA256SUMS"
cp -P -- "$backup_dir/fornacast.dump" "$staging_dir/fornacast.dump"
cp -P -- "$backup_dir/fornacast-data.tgz" "$staging_dir/fornacast-data.tgz"

for artifact in SHA256SUMS fornacast.dump fornacast-data.tgz; do
  if [ ! -f "$staging_dir/$artifact" ] || [ -L "$staging_dir/$artifact" ]; then
    printf '%s\n' "staged backup artifact must be a regular non-symlink file: $artifact" >&2
    exit 1
  fi
done

chmod 600 "$staging_dir/SHA256SUMS"
chmod 600 "$staging_dir/fornacast.dump"
chmod 600 "$staging_dir/fornacast-data.tgz"

manifest_lines=$(wc -l <"$staging_dir/SHA256SUMS" | tr -d ' ')
test "$manifest_lines" = 2
grep -Eq '^[0-9a-f]{64}  fornacast\.dump$' "$staging_dir/SHA256SUMS"
grep -Eq '^[0-9a-f]{64}  fornacast-data\.tgz$' "$staging_dir/SHA256SUMS"
(cd "$staging_dir" && sha256sum -c SHA256SUMS)
compose exec -T db sh -eu -c 'exec pg_restore --list >/dev/null' \
  <"$staging_dir/fornacast.dump"
tar -tzf "$staging_dir/fornacast-data.tgz" >/dev/null

if [ "$confirmation" != "--confirm-destroy" ]; then
  printf '%s\n' "restore is destructive; pass --confirm-destroy after reviewing the backup" >&2
  exit 64
fi
...
docker run --rm -i \
  --mount "type=volume,source=$data_volume,target=/data" \
  alpine:3.22 tar -C /data -xzf - <"$staging_dir/fornacast-data.tgz"
