defmodule ForgeBlobs.ManagerTest do
  ...
  test "the exact instance is ready, owned, and has only its Engine child" do
    ...
    assert Enum.any?(
             DynamicSupervisor.which_children(ForgeBlobs.InstanceSupervisor),
  ...
  test "a cold start failure stops the manager" do
    ...
    assert {:stop, {:blob_storage_start_failed, :instance_supervisor_unavailable}} =
  ...
  test "Engine loss becomes not-ready and recovers inside the same instance" do
    ...
    assert_receive {:blob_storage_status, {:not_ready, :engine_down}}, 1_000
  ...
  test "instance loss becomes not-ready and restarts without application or Concord loss" do
    blob_supervisor = Process.whereis(ForgeBlobs.ApplicationSupervisor)
    ...
    assert_receive {:blob_storage_status, {:not_ready, :instance_down}}, 1_000
    ...
    assert Process.whereis(ForgeBlobs.ApplicationSupervisor) == blob_supervisor
  ...
  test "ESS infrastructure loss recovers without ForgeBlobs or Concord loss" do
    blob_supervisor = Process.whereis(ForgeBlobs.ApplicationSupervisor)
  ...
  test "manager restart during Engine recovery attaches once and becomes ready" do
    ...
    assert :ok =
             Supervisor.terminate_child(ForgeBlobs.Supervisor, Manager)

    assert {:ok, _manager} =
             Supervisor.restart_child(ForgeBlobs.Supervisor, Manager)
