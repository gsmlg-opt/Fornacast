  ...
  def index(%Plug.Conn{assigns: %{current_user: user}} = conn, _params) do
    ...
    rows =
      ...
      |> Enum.map(fn repo ->
        ...
        ~s(<tr><td><a href="/#{escape(owner.username)}/#{escape(repo.slug)}">#{escape(owner.username)}/#{escape(repo.name)}</a></td><td>#{escape(repo.visibility)}</td><td>#{escape(repo.default_branch)}</td></tr>)
      end)
      |> Enum.join("\n")
    ...
    body =
      ...
      <p><a href="/organizations/new">Create an organization</a></p>
      <table>
